    ...
    for col in cols:
        if False:  # seleziono tipo di analisi outliers
            ...
            print("Data points considered step-outliers for the "
                  "col '{}':".format(col))
            df_plot[~((df_outliers[col] >= Q1 - step) &
                      (df_outliers[col] <= Q3 + step))].display()
        else:
            Qmin = np.percentile(df_outliers[col], 2)
            Qmax = np.percentile(df_outliers[col], 98)
            print("Data points considered 2-percent outliers for "
                  "the col '{}':".format(col))
            df_plot[~((df_outliers[col] >= Qmin) &
                      (df_outliers[col] <= Qmax))].display()
