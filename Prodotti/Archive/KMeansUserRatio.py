...
# 14-9-18: salvo questo file perchè è quello che ha generato i cluster
# della prima mail girata, nel caso in cui servisse replicare
...
# Feature Relevance - droppo in base alle osservazioni risultanti del
# ciclo successivo
prod_proc.drop(['nFarma', 'nRight', 'nUsers',
                'LatVar', 'nReg', 'nTot'], axis=1, inplace=True)
prod_feat = prod_proc.drop(['ProductId', 'ProdName'], axis=1)
Features.print_relevance(prod_feat)

# Outliers
if 0:
    Features.print_outliers(prod_proc, prod, prod_feat.keys())
...
samples_proc = prod_proc[prod_proc.ProductId.isin(
    samples)].drop(['ProductId', 'ProdName'], axis=1)
